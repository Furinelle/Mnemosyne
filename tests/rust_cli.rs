...
fn mcp_respects_exposure_and_has_no_stdout_noise() {
    ...
    .unwrap();
    let input=[json!({"id":1,"method":"initialize"}),json!({"id":2,"method":"tools/call","params":{"name":"mnemosyne_read_core","arguments":{"scope":"global"}}}),json!({"id":3,"method":"tools/call","params":{"name":"mnemosyne_search","arguments":{"query":"missing"}}})].iter().map(Value::to_string).collect::<Vec<_>>().join("\n");
...
fn mcp_project_path_works_without_host_project_cwd() {
    ...
    let request = json!({"id":1,"method":"tools/call","params":{"name":"mnemosyne_write","arguments":{"type":"codebase","importance":70,"content":"explicit project only","project_path":project}}});
    ...
    let request = json!({"id":2,"method":"tools/call","params":{"name":"mnemosyne_write","arguments":{"type":"codebase","importance":70,"content":"no implicit root project"}}});
    let reply: Value = serde_json::from_str(&success(run(
        t.path(),
        &["mcp", "serve"],
        &request.to_string(),
