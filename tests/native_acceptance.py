...
with tempfile.TemporaryDirectory(prefix='mnemosyne-acceptance-') as temp:
    ...
    run('eval-scoring',['python3','tests/native_eval_scoring_test.py'])
    run('perf-fixture-tests',['python3','tests/native_perf_test.py'],extra={'MNEMOSYNE_PERF_BINARY':str(root/'target/debug/mnemosyne')})
    run('handoff-runner-tests',['python3','tests/native_handoff_eval_test.py'])
    binary=str(root/'target/debug/mnemosyne')
    run('release-build',['cargo','build','--offline','--locked','--release','--features','onnx'])
    run('handoff-native',['python3','tests/native_handoff_eval.py',binary,'--output',str(output/'handoff-native.json')])
