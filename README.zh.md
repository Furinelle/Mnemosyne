...
v2 包含显式来源、任务检查点、语义修订历史、按系统时间查询、保守 Git
...
2.0.2 补齐 sleep 输出分页及原子收据，支持源变化后的已提交页重放，快照保留已完成
收据，并修正评测质量判分。未完成的 sleep 快照、含 sleep 收据的 fork 会明确拒绝。
自用 V2 基线覆盖 100／1,000 条；离线生产者／消费者验证不等于真实模型或宿主验收。
详见[验收与限制](docs/plans/eval-sleep/ACCEPTANCE.md)。
...
- [源码包](https://github.com/Furinelle/Mnemosyne/releases/download/v2.0.2/mnemosyne-2.0.2-source.tar.gz)
...
v2.0.2 测试套件包含 181 项测试；Linux 与 macOS CI 运行发布检查。本地宿主 hook 检查覆盖
