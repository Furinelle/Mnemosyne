"""Agent-neutral two-way handoff channel: context out, findings back in.

Any agent can consume `prep()` output as a prompt prefix and report new
knowledge with a findings block (markdown or JSON; see docs/handoff-format.md).
"""
...
def prep(
    ...
    if keywords:
        ...
        if results:
            ...
            parts.append(format_for_injection(results, max_tokens=max_tokens, summary_chars=summary_chars))
            parts.append('')
    if channel == 'cli':
        parts.append('### Mnemosyne CLI available')
        parts.append('If you need more context mid-task:')
        parts.append('    python3 -m mnemosyne search "<keywords>" --format json --limit 3')
        parts.append('')
    elif channel == 'mcp':
        parts.append('### Mnemosyne memory tools available')
        parts.append('If you need more context mid-task, call the mnemosyne_search tool')
        parts.append('with your keywords.')
        parts.append('')
