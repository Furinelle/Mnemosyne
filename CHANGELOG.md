...
- Sleep 支持有界输出分页、幂等收据与恢复；提案、收据和进度共用事务，所有输出页提交后才推进输入游标。
- 已提交页可在源变化或提案审批后精确重放，不重置审批状态、不覆盖较新的进度；缺页、改写总页数和异内容重试明确拒绝。
- 快照保留完成的 sleep 收据；未完成的当前批次和含 sleep 收据的 fork 明确拒绝，避免静默丢失恢复状态。
- 评测分别判定执行、输出协议及回答质量，错误答案、缺组、无证据猜测和不合法输出不能通过。

### 验证与范围 (Validation)

- 增加提交后应答丢失、突然中止、分页并发、审批后重放及快照恢复回归。
- 正式 V2-current / V2-evolution 语料覆盖 100／1,000 条、六条路径、每路径五次采样；无性能优化或全面提速声明。
- 两个原生接口方向的八个离线交接场景与 runner 负例进入 CI。真实模型／宿主实验未运行；用户明确不要求一万条评测。
- 公开 Rust 结构体有新增字段；CLI/MCP 的旧单页输入保持兼容。含新 sleep 数据的快照不由旧版本恢复器支持；不承诺旧 writer 与新恢复日志混写。

## [2.0.1] - 2026-09-20

### 修复 (Fixed)

- 升级后的 `ingest`、host/heuristic/LLM 蒸馏与 Stop 使用 V2 来源和修订写入；保留旧 writer 拒绝保护，补全保存与重放回归。
- 升级后预览与提交复用 V2 去重和来源判定；保留搜索结果中的 evidence 摘要。
- Reconciliation 先比较结构化范围与取值，同正文的单值冲突不再误报 SUPPORT。
- 正式修订后的新来源可支持当前事实；旧来源与旧历史保持原关联。
- Sleep 按单页字节预算读取并返回 `partial` / `next_cursor`，不因全库正文超过 4 MiB 拒绝第一页；仍保留单条和目录数量限制。

### 验证与记录 (Validation)

- 原生默认与 all-features 各 159 项回归，另运行隔离协议、模型、升级恢复和打包门禁。
- 增加升级后跨入口写入、修订与来源互操作、冲突分类、分页续读的交叉回归。
- 增加显式启用的分阶段耗时记录，修正原验收记录对范围和未执行项的表述。
- 配置中的真实付费模型调用与四宿主完整模型会话不由协议 fixtures 代替。
